...
export default class Utility {
  ...
  static DataType = {
    ...
    Boolean: 'Boolean',
    Object: 'Object',
    DateTimeByISO601: 'DateTimeByISO601',
    DateTime: 'DateTime'
  };
   ...
   * @memberof Utility
   */
  static formatDate(value, format = 'yyyy-mm-dd HH:MM:ss.S') {
    if (!Utility.isTimestamp(value)) {
      return value;
    }
    let __this = new Date();
    let _fmt = format || 'yyyy-mm-dd HH:MM:ss.S';

    try {
      __this = new Date(value);
    } catch (ex) {
      __this = new Date();
    }

    const oo = {
      'Y+': __this.getFullYear(),        // 月份
      'y+': __this.getFullYear(),        // 月份
    ...
    for (const kk in oo) {
      if (new RegExp(`(${kk})`).test(format)) {
        let _value = oo[kk];
        let __length = RegExp.$1.length === 1 ? 3 : RegExp.$1.length;
        for (let i = 0; i < __length; i += 1) {
          _value = `0${_value}`;
        }
        _fmt = _fmt.replace(RegExp.$1, _value.substr(`${oo[kk]}`.length));
      }
    }
    return _fmt;
  }
  ...
  static generateOrderCode(numLength) {
    const time = Utility.formatDate(new Date().getTime(), 'yyyymmddHHMMss');
   ...
   * @memberof Utility
   */
  static generateSerNum(len = 4) {
    const time = Utility.formatDate(new Date().getTime(), 'yyyymmddHHMMss');
    const randomNum = randomstring.generate({ length: len, charset: 'numeric' });
